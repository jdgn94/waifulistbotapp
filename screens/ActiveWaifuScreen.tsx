...
export default function TabTwoScreen() {
  const [waifusActives, setWaifusActives] = React.useState(
    [] as IwaifusActives[]
  );
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await FetchController.get("waifus/active");
  ...
  const WaifuActive = (active: IwaifusActives) => {
    return (
      <View key={active.id} style={styles.waifuContainer}>
        <Image
        ...
        <Text>
          Name:{" "}
          {active.waifu_name +
            (active.waifu_nickname ? ` - ${active.waifu_nickname}` : "")}
        </Text>
        <Text>
          Franchise:{" "}
          {active.franchise_name +
            (active.franchise_nickname
              ? ` - ${active.franchise_nickname}`
              : "")}
        </Text>
      </View>
